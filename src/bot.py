...
log_info("Loaded all needed imports", startup=True)


CONFIG_PATH = "config.json"
log_info("Loaded config path", startup=True)

log_info("Loading Config", startup=True)
config = load_config(CONFIG_PATH)
log_info("Completed loading config", startup=True)

log_info("Loading default values into Memory", startup=True)
color_manager = ColorManager(config)
log_info("Initialized color manager", startup=True)

FOOTER_TEXT = config["defaults"].get("footer_text")
log_info("Loaded footer text", startup=True)
...
log_info("Loaded bot version", startup=True)

TTS_MODE = config.get("tts_mode", "normal")
log_info("Loaded tts mode", startup=True)

SYSTEM_PROMT = config.get("system_prompt", "none")
log_info("Loaded system prompt", startup=True)

LOGGING_CHANNEL_ID = int(config.get("log_channel_id", 0))
log_info("Loaded logging channel id", startup=True)

intents = disnake.Intents.all()
log_info("Initialized intents", startup=True)

log_info("Initialized command sync flags", startup=True)
...
log_info("Initialized bot", startup=True)
...
console = Console()
log_info("Initialized console", startup=True)

afk_users: dict[int, str] = {}

set_langdetect_seed(config.get("tts_detector_factory_seed", 0))
log_info("Loaded tts detector factory seed", startup=True)
