...
def run_speed_test(console: Console) -> Tuple[float, float, float, str]:
    ...
    try:
        with Progress(
            ...
            progress.update(
                init_task, advance=1, description="Speed test initialized ✓"
            )
            ...
            progress.update(
                download_task,
                advance=1,
                description=f"Download: {format_speed(download)} ✓",
            )

            upload_task = progress.add_task("Testing upload speed...", total=1)
            upload = st.upload()
            progress.update(
                upload_task, advance=1, description=f"Upload: {format_speed(upload)} ✓"
            )
        ...
        log_info(
            "Please ensure you have a working internet connection and try again.",
            error=True,
        )
        sys.exit(1)
...
def main(token: str, skip_speedtest: bool):
    ...
    if not skip_speedtest:
        ...
        if download < 5_000_000 or upload < 1_000_000:
            log_info(
                "Your internet connection appears to be slow, which might affect bot performance.",
                warning=True,
            )
